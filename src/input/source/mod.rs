...
pub trait SourceInputDevice {
    ...
    /// Returns file descriptors that become readable when new data is available.
    /// When non-empty, SourceDriver will use poll() on these fds instead of
    /// fixed-rate sleeping, enabling data-driven event delivery.
