...
pub fn find_trigger(ctx: &Context, sample_rate: f64) -> Option<Device> {
    ...
    for i in 0..num_devices {
        ...
        if name.contains("hrtimer") {
            if let Err(e) = dev.attr_write("sampling_frequency", sample_rate as i64) {
                log::warn!("Failed to set hrtimer sampling frequency to {sample_rate}: {e}");
            }
    ...
    if let Some(ref trig) = fallback_trigger {
        ...
        if let Err(e) = trig.attr_write("sampling_frequency", sample_rate as i64) {
            log::warn!("Failed to set trigger sampling frequency to {sample_rate}: {e}");
        }
